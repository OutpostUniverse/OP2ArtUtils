    ...
    def LoadPAL(self, filename, file_format=TEXT):
        ...
        with open(filename, 'rb') as f:
            if file_format == PAL:
                ...
                if pal_tag != b'PAL data':
                    raise PaletteLoadError('PAL data section is missing')
                if pal_ver != 0x0300:
                    raise PaletteLoadError(f'Expected version 0x0300, got {pal_ver}')
                if pal_size != pal_colors * 4 + 4:
                    raise PaletteLoadError(f'Invalid palette size ({pal_size}, expected {pal_colors * 4 + 4})')
    ...
    def WritePAL(self, filename, file_format=TEXT):
        ...
        with open(filename, 'wb') as f:
            if file_format == PAL:
                ...
                data_header = PAL_HEADER.pack(b'PAL data', len(self.colors) * 4 + 4, 0x0300, len(self.colors))
                riff_header = RIFF_HEADER.pack(b'RIFF', len(data_header))
                f.write(riff_header)
                f.write(data_header)
