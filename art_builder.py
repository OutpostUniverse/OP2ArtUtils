...
def main():
    ...
    parser.add_argument('--palette_format', default='pal', choices=palette.PAL_FORMATS.keys(), help='Palette format to read.  Choices are pal (Microsoft RIFF .pal format), text (.pal text format supported by Paint Shop Pro), or act (Photoshop .act color table).')
    parser.add_argument('--palettes_from_bitmaps', action='store_true', default=False, help='Reads palettes directly from input bitmaps if set, ignoring the palette files and metadata.  This will result in a unique palette entry for each loaded bitmap in the PRT file, which will increase its size as well as the amount of time it takes Outpost 2 to load the data, but allowing for greater flexibility with colors.')
    ...
    with args.prt as prt:
        with args.bmp as bmp:
            ...
            with open(os.path.join(base_path, 'bitmaps.yml'), 'r') as f:
                ...
                for k in sorted(bmp_metadata.keys()): 
                    v = bmp_metadata[k]
                    bmp = bitmap.Bitmap()
