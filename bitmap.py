...
class Bitmap:
    ...
    def LoadBMP(self, filename):
        ...
        with open(filename, 'rb') as f:
            ...
            if pal_len != 256:
                raise BitmapLoadError(f'unexpected palette length (got {pal_len}, expected 256)')
            if bpp not in [1, 8]:
    ...
    def WriteBMPToOpenFile(self, f):
        ...
        file_header = BITMAPFILEHEADER.pack(b'BM', 14 + len(info_header) + len(self.palette.colors) * 4 + len(self.data), 0, 0, 14 + len(info_header) +
